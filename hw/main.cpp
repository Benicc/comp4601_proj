...
int main() {
    ...
    uint32_t *result_buffer = (uint32_t *)aligned_alloc(ALIGNMENT, SIZE * sizeof(uint32_t));
    assert(((u64)(UINTPTR)result_buffer) % ALIGNMENT == 0);
    Xil_DCacheFlushRange((UINTPTR)result_buffer, 8 * sizeof(uint32_t));
    ...
    XData_mover_Set_result_mem(&data_mover, (u64)(UINTPTR)result_buffer);
    // start data loader
    ...
    Xil_DCacheInvalidateRange((UINTPTR)result_buffer, 8 * sizeof(uint32_t));
    ...
    for (int i = 7; i >= 0; i--) {
        xil_printf("%08x ", result_buffer[i]);
    }
    xil_printf("\r\n");
